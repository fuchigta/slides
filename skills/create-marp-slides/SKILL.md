...
3. **日本語テキスト注意点**:
   ...
   - 長文は箇条書きに分解する
   - 日本語の改行位置に注意する。複合語や名詞句の途中で改行されないよう、テキストを短くして1行に収めるか、自然な区切り（読点の後、助詞の後、節の境界）に`<br>`を挿入して改行位置を制御する
...
- **日本語の改行位置**: 複合語や名詞句の途中で不自然に改行されていないか。特に幅が狭いコンテナ（icon-grid, comparison-table, sidebar等）に注意
- **余白・間隔のバランス**
